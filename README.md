...
基于 [coolsnowwolf/lede](https://github.com/coolsnowwolf/lede)（master 分支，OpenWrt 24.10.x，当前默认版本 24.10.5，与你要求的 24.10.3 同系列且更新）的 GitHub Actions 云编译配置，一套仓库编译**两套互不混用的固件**：

| 固件 | 目标平台 | 适用设备 | 管理IP |
|---|---|---|---|
| `x86` | x86_64 | Intel J1900 / J1800 软路由（通用 Generic 镜像） | `10.0.1.1` |
| `ax3000t` | mediatek/filogic | 小米 AX3000T（MT7981B，精简版） | `10.0.0.1` |
...
| 需求 | 实际包 | 来源 |
|---|---|---|
| passwall | `luci-app-passwall`（v26.9.1，含 xray + sing-box 核心） | Lede luci feed 自带（避免与外部仓库重复冲突） |
| passwall2 | `luci-app-passwall2`（All 核心） | `Openwrt-Passwall/openwrt-passwall2` |
| 易有云文件管理器 | `luci-app-linkease` | `linkease/luci-app-linkease`（你清单外，自行找到） |
| 1Panel | 无 luci 插件 → 见下方【1Panel 说明】 | — |
| 全能推送 | `luci-app-pushbot` | `zzsj0928/luci-app-pushbot`（你清单外，自行找到） |
| 微信推送 | `luci-app-wechatpush` | `tty228/luci-app-serverchan`（**包名实际为 wechatpush**） |
| DDNS-GO | `ddns-go` + `luci-app-ddns-go` | `sirpdboy/luci-app-ddns-go`（仓库内同时含二进制与界面） |
| Lucky | `lucky` + `luci-app-lucky` | `sirpdboy/luci-app-lucky`（适配 24.10） |
| OAF 行为管理 | `luci-app-appfilter` + `appfilter` + `kmod-oaf` | **Lede feeds 自带**（packages feed 提供 kmod-oaf/appfilter，luci feed 提供 luci-app-appfilter；外部 destan19 仓库与 feed 同名冲突，故不克隆） |
...
| iStore 商店（含中文包） | `luci-app-store` + `luci-lib-taskd` + `taskd` + `luci-i18n-store-zh-cn` | `linkease/istore` |
...
| alist 文件列表 / openlist | `luci-app-openlist`（OpenList 是 Alist 的继任者，**Provides: luci-app-alist**） | Lede luci feed 自带（sbwml 的 alist 仓库已归档，故不再重复添加，避免包名冲突） |
...
- **WiFi 2.4G / 5G**：默认包含 mt7981 固件，信道、频段带宽在 LuCI「无线」页面直接修改
- **IP 限速**：`luci-app-eqosplus`
- **passwall**（精简：仅内置 sing-box 核心，xray 等核心可在 passwall 内在线下载，适配 128MB 闪存）
...
| 03:50（=UTC 19:50） | 兜底：检查当天是否已触发构建；**若 03:45 清理失败未触发 → 自动触发构建** |
